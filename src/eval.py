...
evaluated_data_roots = ['./test-other_ordered']
...
model = model.load_from_checkpoint(checkpoint_path='./epoch=59-step=527340.ckpt',
                                   tokenizer=tokenizer,
                                   text_classes=tokenizer.num_tokens(),
                                   device=params.device)
...
for evaluated_data_root in evaluated_data_roots:
    ...
    for eval_batch in tqdm(evaluation_dataloader, desc=f'Evaluating {evaluated_data_root}'):
    ...
    print(f"Evaluated dataset: {evaluated_data_root}")
    print(f"Avg. WER: {wer}")
    print(f"Avg. CER: {cer}")
